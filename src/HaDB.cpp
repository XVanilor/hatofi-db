...
void HaDB::load(const std::string& file)
{
    ...
    while(std::getline(ifs, line))
    {
        ...
        std::string dataOutFileName = fmt::format("{}/dataclass/{}/{}/{}/{}.crypto", this->getRoot(), columns[1], columns[3].substr(0,2), columns[3].substr(2,2),columns[3]);
        dataOutFile.open(dataOutFileName);
        dataOutFile << fmt::format("b64:{}\n", columns[2]);
        dataOutFile.close();

        std::string entityOutFileName = fmt::format("{}/entity/{}/{}/{}.crypto", this->getRoot(), columns[0].substr(0,2), columns[0].substr(2,2),columns[0]);
...
std::string HaDB::query(std::string dataclass, std::string searchString, HaDB::MATCH_TYPE matchType)
{
